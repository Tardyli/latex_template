...
\section{实验环境与证据材料}
本文实验使用 IXI T1 脑部 MRI 数据作为主要数据来源。原始数据为 NIfTI 体数据，经过预处理后转换为 PNG 二维切片，并进一步打包为 StyleGAN2 训练所需的数据集。根据训练配置文件，StyleGAN2 使用的数据集为 \texttt{mri128.zip}，最大训练切片数量为 60815 张，图像分辨率为 $128\times128$。3DGS 实验以连续 MRI 切片序列作为三维训练输入，包含 180 张 $256\times256$ 切片，并使用 300000 个初始点作为三维高斯优化起点。

为了使实验结论能够追溯到真实项目产物，本文将训练配置、日志文件、模型权重、生成切片、点云文件、3DGS checkpoint、渲染图像和指标统计结果作为主要证据材料。这些材料均来自项目源码目录和结果目录，覆盖数据处理、二维生成、三维重建和平台测试的关键环节。
...
StyleGAN2 训练采用单 GPU 配置，batch size 设置为 4，配置训练量为 25000 kimg。生成器和判别器均使用 Adam 优化器，学习率为 0.0025，潜变量维度和中间潜变量维度均为 512。训练过程中启用 ADA 数据增强，目标值为 0.6。训练输出目录中保存了训练配置、训练日志、真实样本网格以及模型权重快照。
...
训练样本网格如图 \ref{fig:stylegan2-reals} 所示。该图来自训练输出目录中的 \texttt{reals.png}，用于确认训练集已经被正确读取，并且输入样本为单通道脑部 MRI 切片。样本网格中可以观察到较清晰的脑部轮廓和灰度层次，说明预处理后的切片能够为二维生成模型提供稳定输入。
...
训练完成后，项目得到可加载的模型快照 \texttt{network-snapshot-010000.pkl}。平台能够读取该权重并生成 MRI 切片，说明权重加载、潜变量采样、生成器推理、图像张量转换和 PNG 保存等环节已经连通。本文对 StyleGAN2 结果的评价重点放在生成链路可用性、输出可复现性和对三维重建输入的支撑能力上。

\section{二维切片生成结果分析}
平台生成的示例切片如图 \ref{fig:generated-slices} 所示。该图来自生成切片结果目录，由训练完成的 StyleGAN2 权重生成。不同随机种子对应不同潜变量输入，因此可得到不同灰度分布和结构形态的 MRI 切片示例。图中结果能够呈现脑部 MRI 切片的基本轮廓和灰度结构，说明训练权重已能被平台推理模块正确调用。
...
从系统角度看，二维切片生成链路已经形成闭环。用户在平台中指定模型权重、生成数量、截断参数和随机种子后，系统自动完成潜变量采样、生成器前向推理、灰度图像转换和结果保存。截断参数 \texttt{truncation\_psi} 可调节生成结果的稳定性与多样性，随机种子则保证同一参数组合下结果可复现。该功能既可产生论文展示图，也可作为连续切片序列和点云初始化的输入来源。

\section{平台功能测试}
平台功能测试主要验证数据预处理、模型训练入口、切片生成、连续序列生成、点云初始化、3DGS 结果展示和 AI 助手等流程是否能够按照预期运行。测试结果显示，平台主页、参数输入组件和 Gradio 后端服务可以正常访问；生成模块能够调用训练完成的权重输出 PNG 切片；点云初始化模块能够从切片目录生成 PLY 文件；3DGS 训练结果能够以日志、渲染图像和点云文件形式归档并用于论文展示。
...
综合测试结果可知，平台已经具备毕业设计所需的完整演示能力。使用者可以通过界面理解并操作从数据预处理到二维生成、再到初始三维表示和 3DGS 重建结果展示的主要流程。平台首页、预处理入口、生成结果画廊、点云初始化日志、3DGS 结果路径和异常提示共同构成系统展示材料，能够支撑论文验收和答辩演示。
...
该点云体现了二维切片向三维空间组织的基本过程：每张切片对应一个 $z$ 轴位置，非背景像素对应空间点，像素灰度对应颜色强度。因此，点云既能作为 3DGS 三维重建输入，也能作为平台 2D/3D 对比展示的中间结果。与只展示单张二维生成图相比，该步骤为三维结构表达提供了明确的工程接口。
...
训练过程中，系统在第 1000、3000、5000、7000、15000 和 30000 轮记录训练集重建指标，并保存对应高斯点云。表 \ref{tab:3dgs-metrics} 给出了训练日志中记录的主要指标。可以看到，随着迭代推进，L1 误差整体下降，PSNR 整体提高，说明三维高斯参数在切片监督下逐步逼近目标 MRI 切片分布。
...
第 30000 轮渲染切片与参考切片对比如图 \ref{fig:3dgs-comparison-30000} 所示。图中选取序列前部、中部和后部切片进行展示。可以观察到，渲染结果在脑部轮廓、主体灰度区域和层间变化趋势上与参考切片保持一致。局部细节仍存在一定平滑化现象，这与 MRI 灰度纹理、单序列输入约束和当前训练规模有关。
...
结合训练日志、checkpoint、点云文件和渲染结果可以确认，3DGS 模块已经完成从规则切片序列到三维高斯表达的训练闭环。输入点云从 300000 点优化为 572073 个高斯点，说明训练过程在初始空间结构基础上进一步调整和扩展了三维表达。该结果能够支撑平台进行 MRI 图像三维重建展示，也为后续更高质量的医学三维可视化研究提供基础。
...
系统仍存在进一步提升空间。第一，StyleGAN2 生成结果目前主要通过可视化和工程链路验证，若后续加入更大规模生成样本统计、专家评价或下游任务评价，可进一步增强医学合理性分析。第二，3DGS 训练基于规则切片相机和单序列灰度图像，虽然能够形成稳定的三维表达，但对细粒度组织边界和复杂解剖结构的表达仍有提升空间。第三，平台部分功能依赖本地路径、GPU 环境和外部 API，跨环境部署能力可通过配置文件、环境变量和容器化方式继续增强。上述局限不影响本文系统完成主要设计目标，但为后续扩展研究提供了明确方向。
