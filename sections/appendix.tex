...
本文涉及的工程代码与实验结果主要保存在项目目录 \thesispath{MRI_Platform} 中。平台入口文件为 \thesispath{src/app.py}。该文件用于构建 Gradio 交互界面，并调度模型权重查找、切片生成、连续序列构建和点云初始化等功能。MRI 数据预处理脚本为 \thesispath{preprocess_mri.py}。该脚本负责读取 NIfTI 体数据并导出 PNG 切片。点云初始化脚本为 \thesispath{init_pcd.py}。该脚本负责把连续切片序列转换为 PLY 点云文件。
